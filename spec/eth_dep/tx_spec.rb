describe Eth_DEP::Tx, type: :model do
  ...
  describe "#initialize" do
    ...
    context "when the gas limit is too low" do
      ...
      it "raises an InvalidTransaction error" do
        expect { tx }.to raise_error(Eth_DEP::InvalidTransaction, "Gas limit too low")
      end
    end

    context "there are values beyond the unsigned integer max" do
      let(:nonce) { Eth_DEP::UINT_MAX + 1 }

      it "raises an InvalidTransaction error" do
        expect { tx }.to raise_error(Eth_DEP::InvalidTransaction, "Values way too high!")
      end
    end
  ...
  describe ".decode" do
    ...
    it "returns an instance that matches the original enocded one" do
      tx2 = Eth_DEP::Tx.decode tx1.encoded
      expect(tx2).to eq(tx1)
    end

    it "also accepts hex" do
      tx2 = Eth_DEP::Tx.decode(tx1.hex)
      expect(tx2).to eq(tx1)
    end
  end
  ...
  describe "#hash" do
    ...
    it "hashes the serialized full transaction" do
      txids.each do |txid|
        tx = Eth_DEP::Tx.decode read_hex_fixture(txid)
  ...
  describe "#data_hex" do
    it "converts the hex to binary and persists it" do
      ...
      binary = Eth_DEP::Utils.hex_to_bin hex
  ...
  describe "#data" do
    ...
    let(:binary) { Eth_DEP::Utils.hex_to_bin hex }
