...
describe Eth_DEP::Utils, type: :model do
  describe ".int_to_base256" do
    ...
    let(:bin) { Eth_DEP::Utils.hex_to_bin hex }
    let(:int) { Eth_DEP::Utils.base256_to_int bin }

    it "gets the same result back" do
      base256 = Eth_DEP::Utils.int_to_base256 int
  ...
  describe ".base256_to_int" do
    it "properly converts binary to integers" do
      expect(Eth_DEP::Utils.base256_to_int("\xff")).to eq(255)
      expect(Eth_DEP::Utils.base256_to_int("\x00\x00\xff")).to eq(255)
    end
  end

  describe ".prefix_hex" do
    it "ensures that a hex value has 0x at the beginning" do
      expect(Eth_DEP::Utils.prefix_hex('abc')).to eq('0xabc')
      expect(Eth_DEP::Utils.prefix_hex('0xabc')).to eq('0xabc')
    end

    it "does not reformat the hex or remove leading zeros" do
      expect(Eth_DEP::Utils.prefix_hex('0123')).to eq('0x0123')
    end
  end
  ...
  describe ".public_key_to_addres" do
    ...
    it "turns a hex public key into a hex address" do
      expect(Eth_DEP::Utils.public_key_to_address(pub)).to eq(address)
    end
  end
  ...
  describe ".keccak256" do
    it "properly hashes using" do
      ...
      expect(value). to eq(Eth_DEP::Utils.keccak256(''))
    end
  end
  ...
  describe ".keccak256_rlp" do
    it "properly serializes and hashes" do
      ...
      expect(value1).to eq Eth_DEP::Utils.keccak256_rlp('')
      expect(value2).to eq Eth_DEP::Utils.keccak256_rlp(1)
      expect(value3).to eq Eth_DEP::Utils.keccak256_rlp([])
      expect(value4).to eq Eth_DEP::Utils.keccak256_rlp([1, [2,3], "4", ["5", [6]]])
    end
  end
  ...
  describe ".hex_to_bin" do
    it "raises an error when given invalid hex" do
      expect {
        Eth_DEP::Utils.hex_to_bin('xxxx')
      }.to raise_error(TypeError)

      expect {
        Eth_DEP::Utils.hex_to_bin("\x00\x00")
      }.to raise_error(TypeError)
    end
  end
  ...
  describe ".ripemd160" do
    it "properly hashes with RIPEMD-160" do
      ...
      expect(value).to eq Eth_DEP::Utils.ripemd160("\x00")
    end
  end

  describe ".format_address" do
    let(:address) { "0x5AAEB6053F3E94C9B9A09F33669435E7EF1BEAED" }
    subject { Eth_DEP::Utils.format_address address }
