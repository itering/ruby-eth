describe Eth_DEP::Key, type: :model do
  let(:priv) { nil }
  subject(:key) { Eth_DEP::Key.new priv: priv }

  describe "#initialize" do
    it "returns a key with a new private key" do
      key1 = Eth_DEP::Key.new
      key2 = Eth_DEP::Key.new
    ...
    it "regenerates an old private key" do
      key1 = Eth_DEP::Key.new
      key2 = Eth_DEP::Key.new priv: key1.private_hex
  ...
  describe "#sign" do
    ...
    it "signs a message so that the public key is recoverable" do
      10.times do
        ...
        s_value = Eth_DEP::Utils.v_r_s_for(signature).last
        expect(s_value).to be < (Eth_DEP::Secp256k1::N/2)
      end
    end

    context "when the network ID has been changed", chain_id: 42 do
      it "signs a message so that the public key is recoverable" do
        v_base = Eth_DEP.v_base
        expect(v_base).to eq(119)

        10.times do
          signature = key.sign message
          expect(key.verify_signature message, signature).to be_truthy
          v_val, r_val, s_val = Eth_DEP::Utils.v_r_s_for(signature)
          expect(s_val).to be < (Eth_DEP::Secp256k1::N/2)
  ...
  describe "#verify_signature" do
    ...
    context "when the network ID has been changed", chain_id: 42 do
      ...
      it "can verify signatures from the new network" do
        v = Eth_DEP::Utils.v_r_s_for(new_signature).first
      ...
      it "can verify replayable signatures" do
        v = Eth_DEP::Utils.v_r_s_for(signature).first
  ...
  describe ".encrypt/.decrypt" do
    ...
    let(:key) { Eth_DEP::Key.new }

    it "reads and writes keys in the Ethereum Secret Storage definition" do
      encrypted = Eth_DEP::Key.encrypt key, password
      decrypted = Eth_DEP::Key.decrypt encrypted, password
